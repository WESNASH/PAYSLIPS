...
def send_email(employee_email, employee_name ,payslip_path):
    yag = yagmail.SMTP("nyabungawesley@gmail.com","Tadiwanash21")
    yag.send(
        to=employee_email,
...
for index, employee in data.iterrows():
    ...
    payslip_path = f"{employee_name}_payslip.pdf"
    send_email(employee_email, employee_name, payslip_path)
    print(f"Payslip emailed to {employee_name}")
